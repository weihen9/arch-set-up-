...
if [[ "$GPU_VENDOR" == "nvidia" ]]; then
    nvidia-smi &>/dev/null \
        ...
        || check_fail "nvidia-smi failed — see Troubleshooting in README"
    ...
    lsmod | grep -q "^amdgpu " \
        && check_pass "amdgpu module active" \
        || check_fail "amdgpu not active after reboot"

    lspci -k | grep -A3 -Ei 'vga|3d controller' | grep -qi amdgpu \
        && check_pass "amdgpu bound to GPU (confirmed via lspci -k)" \
        || check_warn "amdgpu not showing as active driver in lspci -k"
else
...
pgrep -x swaync &>/dev/null \
    && check_pass "swaync notification center running" \
    || check_warn "swaync not running"
